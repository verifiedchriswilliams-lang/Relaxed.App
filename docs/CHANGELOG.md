...
- **The bed no longer swells and drops under every spoken line.** The duck went
  from ~5 dB (recovering to full in any 2s+ pause) to a barely-perceptible ~1 dB
  dip with slow ramps that only breathes back up in the longer pauses. On steady,
  broadband beds (white / green / brown noise) the old depth was audible as a
  distracting swell-and-drop; the voice already sits ~9 dB over the bed, so the bed
  now reads as a constant floor with no audible modulation.
